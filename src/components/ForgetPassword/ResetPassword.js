...
const ResetPassword = () => {
  ...
  const { referenceCode: paramCode } = useParams();
  ...
  // ✅ ดึง referenceCode จาก path หรือ query string
  useEffect(() => {
    let code = paramCode;
    if (!code) {
      const params = new URLSearchParams(location.search);
      code = params.get("referenceCode");
    }
    if (code) {
      setReferenceCode(code);
    } else {
      message.error("ไม่พบ reference code");
      navigate("/login-moph");
    }
  }, [paramCode, location, navigate]);
  ...
  const onFinish = async (values) => {
    ...
    try {
      const res = await axios.post(
        `${BASE_URL}/staff/auth/change/forget/password`,
        { referenceCode, newPassword: values.newPassword },
        { headers: { "Content-Type": "application/json" } }
      );

      if (res.data?.statusCode === 200) {
        message.success("รีเซ็ตรหัสผ่านสำเร็จ! กรุณาเข้าสู่ระบบใหม่");
        navigate("/login-moph");
      } else {
        message.error(res.data?.message || "ไม่สามารถรีเซ็ตรหัสผ่านได้");
      }
    } catch (err) {
